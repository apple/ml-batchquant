...
from qfa.layers import IdentityLayer, MBInvertedConvLayer, ConvLayer, LinearLayer
from qfa.imagenet_codebase.utils import MyNetwork, MyModule, int2list, get_net_device, build_activation
from qfa.elastic_nn.modules.dynamic_op import *
from qfa.elastic_nn.utils import adjust_bn_according_to_idx, copy_bn


class DynamicMBConvLayer(MyModule):
    
    def __init__(self, in_channel_list, out_channel_list,
                 kernel_size_list=3, expand_ratio_list=6, stride=1, dilation=1, act_func='relu6', use_se=False):
        ...
        self.depth_conv = nn.Sequential(OrderedDict([
            ('conv', DynamicSeparableConv2d(max_middle_channel, self.kernel_size_list, self.stride, self.dilation)),
            ('bn', DynamicBatchNorm2d(max_middle_channel)),
            ('act', build_activation(self.act_func, inplace=True))
        ]))
        if self.use_se:
